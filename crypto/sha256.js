export class SHA256 {
    ...
    update(data) {
        ...
        if (this._bufLen > 0) {
            ...
            const copy = Math.min(space, data.length);
        ...
        while (off + 64 <= data.length) {
            this._transform(data.subarray(off, off + 64));
            off += 64;
        }

        if (off < data.length) {
            const remaining = data.length - off;
            this._buf.set(data.subarray(off), 0);
    ...
    hexdigest() {
        ...
        let len = this._bufLen;
        buf.set(this._buf.subarray(0, len));
        buf[len] = 0x80;
        len++;
        const bitLen = this._totalLen * 8;
        while ((len % 64) !== 56) {
            if (len === 128) {
                this._transform(buf.subarray(0, 64));
                len = 0;
            }
            buf[len] = 0;
            len++;
        ...
        if (len + 8 > 128) {
            while (len < 128) { buf[len] = 0; len++; }
            this._transform(buf.subarray(0, 64));
            len = 0;
            while (len < 56) { buf[len] = 0; len++; }
        }
        buf[len] = (bitLen >>> 56) & 0xff; len++;
        buf[len] = (bitLen >>> 48) & 0xff; len++;
        buf[len] = (bitLen >>> 40) & 0xff; len++;
        buf[len] = (bitLen >>> 32) & 0xff; len++;
        buf[len] = (bitLen >>> 24) & 0xff; len++;
        buf[len] = (bitLen >>> 16) & 0xff; len++;
        buf[len] = (bitLen >>> 8) & 0xff; len++;
        buf[len] = bitLen & 0xff; len++;

        if (len === 64) this._transform(buf.subarray(0, 64));
        else if (len === 128) {
            this._transform(buf.subarray(0, 64));
            this._transform(buf.subarray(64, 128));
        }
