...
- ✅ **Buffered full-NCA contentId hash — the buffered option IS this feature** — `fs/nca-pack.js` `preparePlaintextProgramNca`, `fs/update.js` `updateMode: 'buffered'`. The merged RomFS + ExeFS live in RAM in the buffered route, and `preparePlaintextProgramNca` precomputes the full-NCA contentId **from the buffer** (hash precompute, no re-stream). The only deferred sub-item is swapping its streaming hash (native node:crypto in Node / ported pure-JS in browser, #79/#80) for a WebCrypto one-shot `crypto.subtle.digest()` (bench: 439 ms vs 4441 ms old JS) — that requires assembling the NCA layout into one temporary 699 MB contiguous buffer (+700 MB RAM peak for ~0.5 s). Deferred by user decision 2026-09-07; enable when memory headroom is confirmed. The port (#79) cut the JS path from 197 → 258 MB/s and Node now streams natively (~1900–2900 MB/s, #80), shrinking the delta a WebCrypto one-shot would buy.
