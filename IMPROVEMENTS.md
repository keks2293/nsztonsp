...
- ❌ **Buffered full-NCA contentId hash → WebCrypto one-shot (deferred, "option 2" for now)** — `fs/nca-pack.js` `preparePlaintextProgramNca`. The ~700 MB contentId hash stays streaming (native node:crypto in Node / ported pure-JS in browser, #79/#80) because a one-shot `crypto.subtle.digest()` (bench: 439 ms vs 4441 ms old JS) requires assembling the NCA layout into one temporary 699 MB contiguous buffer (+700 MB RAM peak for ~0.5 s). Deferred by user decision 2026-09-07; enable when memory headroom is confirmed (peak would be ~1.4 GB in SW+buffered). The port (#79) cut the JS path from 197 → 258 MB/s and Node now streams natively (~1900–2900 MB/s, #80), shrinking the delta a WebCrypto one-shot would buy.
...
- ❌ **hash-wasm WASM — reverted** — `crypto/sha256.js`, `static/hash-wasm.mjs`. Single hash-wasm WASM instance, lazy init. `sha256()`: update+digest+reset. Falls back to pure JS if WASM unavailable. Streaming class `SHA256` remains sync/pure-JS for `converter.js` adapter pattern.
  ...
  - **Removed entirely (2026-09-08, #79)**: зависимость удалена из `package.json`. Повторная (инкрементальная WASM) интеграция отвергнута теми же замерами: at больших чанках WASM всего ~1.36× портированного JS (#79, 248 vs 182 MB/s) и копирует каждый чанк в WASM-heap; Node и так стримит нативно (#80).

## Speed Optimization

- ❌ **FSA (FileSystemWritableFileStream) output can't use the seekable streaming path — stays on two-pass** — `fs/adapter.js:buildRead`, `fs/update.js:697`. The streaming single-decompression update path needs to re-read the written Program NCA to compute its contentId. FSA writables have `seek()` but no `read()` (worker doesn't help — API-level, not threading); OPFS `createSyncAccessHandle()` would (sync read/write in a worker) but staging to OPFS + final copy was **declined**. A "live contentId hash" (hash while writing, no re-read) is **impossible** for the NCA layout: contentId = SHA-256 over the NCA in file order (header → htable → exefs → pad → levels → romfs → pad) and the header/hash-tables contain data-derived hashes (exeHash, romIvfc), so the prefix is only known after the data streamed — SHA-256 can't prepend a late prefix (`fs/nca-pack.js:packProgramNcaStream` tail, ~line 1130+). Blob/memory and Node-fd outputs keep the fast streaming path; SW download stays 3-pass (header-first append-only).
- ❌ **Streaming decode micro-wins A/B'd and reverted — no gain** — `crypto/zstddec-stream-wrapper.js`. (1) Output buffer 1 MiB instead of `ZSTD_DStreamOutSize()` (128 KiB): 1928ms vs 1888ms (−2%, bigger mallocs). (2) Reused grow-only input buffer instead of per-chunk `malloc/free` (16 MiB chunks): 1983ms vs 1888–1929ms (−0..3%, dlmalloc already reuses freed blocks). Both byte-identical; both slower within noise → reverted.

- ✅ **Custom `-Os` zstd decoder in `static/zstddec.mjs`** — `static/zstddec.mjs`. **This file is now a custom rebuild, NOT a copy of `node_modules/zstddec`** — do not "refresh" it from npm. Rebuilt from `facebook/zstd` **v1.5.7** `build/single_file_libs/zstddeclib.c` with **emcc 6.0.9** and `-Os`. Same API/export names as the upstream `zstddec-stream` build (which is what consumers import), so no code changes were needed:
    ```shell
    emcc --no-entry -Os -sALLOW_MEMORY_GROWTH=1 zstddeclib.c \
      ...
      -o zstddec-Os.wasm
    ```
    then substitute the base64 into a copy of the upstream `zstddec-stream` ESM wrapper (the only quoted `[A-Za-z0-9+/=]{1000,}` string is the wasm). wasm 59.8 KB (+1.9 KB vs the npm `-Oz` 59.3 KB), byte-identical output (full 700,338,688 B payload hash match). **Why `-Os`, not `-O3`**: measured E2E full-payload `decode()` — `-O3` 69.4 KB / 1764ms vs `-Os` 59.8 KB / 1688ms (equal speed, 6× cheaper): real `decode()` time is dominated by heap growth + JS chunk copies, so the C-level `-O3` win (raw one-shot: 514 vs 430 MB/s) doesn't survive. `-O2` 68.0 KB / 1856ms. Useful scratch artifacts were in the temp dir (zstd 1.5.7 tarball + wasm variants + `measure_whole.mjs`/`verify_swap.mjs` harnesses) — rebuild from the commands above. Real-file check (Little Nightmares II 4.99 GB, `FORCE_WASM=1`, best-of-3, dev-null): 10950→10643ms (466.9→480.3 MB/s, ×1.03; native node:zlib 10516ms, so WASM went from −3.7% to −1% of native). Full-payload update.nsz: 1954→1688ms (358→415 MB/s, ×1.16).
...
- ✅ **`writeMember`/`writeFromReader` duplicate dispatch + `isNcz` boolean left after #45** — `fs/convert-common.js`, `fs/nsz-convert.js`, `fs/xcz-convert.js`, `fs/container.js`, `fs/update.js`. Entry #45 converted `writeFromReader` and merge/update members to a `kind` discriminated union, but the nsz/xcz path still used a separate `writeMember` (with `isNcz` boolean flag) — two near-duplicate write-dispatch functions in the same file. **Фикс**: `collectFileMetas` now emits the same `kind` discriminator as `writeFromReader`; `writeMember` deleted; all four pipelines call the single `writeFromReader(adapter, writePos, member, progress, verifyOpts?)`. The last shipped `isNcz` flags are gone too: update `otherNcas` src uses `kind`+`srcLen` (was `isNcz`+`ncaLen`), `container.js` `enrichEntry` no longer stamps an unread `isNcz` field (only `outputName` remains), and the boolean locals are gone — `collectFileMetas` derives `kind` straight from the extension, `extractNcaSections` takes a `kind` arg, update source selection branches on `kind === 'ncz'`. No `isNcz` identifier remains. Copy verification now streams per-chunk (was full-buffer read + `sha256`). `sha256` import removed from `convert-common.js`.
