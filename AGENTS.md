...
- **Problem**: `postMessage(data, [data.buffer])` detaches the caller's buffer. Detaching is not just a length-metadata problem — a transferred chunk is emptied, so any later use of that chunk (or of a sibling view sharing the same backing buffer) on the main thread silently sees 0 bytes. This corrupted streaming output by ~1.25 MB of zeros: the pipeline used a chunk after `write()` (`fs/nca-pack.js` streamed `ivfc.update(chunk)` right after the SW/OPFS write) and decode-emits shared backing buffers across chunks.
  **Solution**: (1) In `packProgramNcaStream` hash the chunk **before** the write (`pfs0.update`/`ivfc.update` moved ahead of `adapter.write`, `fs/nca-pack.js`), so transferred chunks are never touched again on this thread — the two-pass writer already hashed before writing, and the real SW path (append-only → two-pass, `SWDownloader.write` slices subarrays and transfers only full buffers, `sw-downloader.js`) has no post-write chunk use left — the reorder closed the last streaming hazard there too. (2) `OpfsOutput.write` (`opfs-output.js`) transfers a buffer **only when it is fully owned** (`view.byteLength === view.buffer.byteLength` — BKTR decrypt outputs, built headers, hash levels are single-use, so this is a true zero-copy transfer); **subarray views are always `slice(0)` first** because their parent may be a pooled source/decompression buffer that must not be detached. This mirrors the JS-web-workers "transfer only what you constructed for the call" rule — a subarray's parent is, by construction, not constructed for the write call.
