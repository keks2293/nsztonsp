 # NSZ to NSP Converter - Status Report
   ...
   - `scripts/cmp_nsp_top.mjs` (added here) uses the cwd-independent run-from-root `prod.keys` loader: `KeysParser.parse(fs.readFileSync(new URL('../static/prod.keys', import.meta.url), 'utf8'))`.
    ...
    14. **Fix: outer NSP PFS0 padding — header 0x20 → 0x10 alignment (yanu/Nintendo parity, −16 B)** — `fs/pfs0.js`, `fs/update.js`. The 16-byte total-size difference vs the yanu reference (701,770,528 vs 701,770,512) was the top-level PFS0 padding rule. Verified on **two original Nintendo NSZs** (Stardew `0100E65002BB8000` and Little Nightmares II `010097100EDD6000`) that Nintendo uses **two different rules**:
       - **outer NSP/NSZ PFS0**: the TOTAL header is aligned to **0x10** (Stardew hdr=0x1D0, LN2 hdr=0x190 — both `mod 0x20 = 16`; the hacpack strtab→0x20 rule would give 0x1E0/0x1A0 — wrong);
       - **inner CNMT PFS0** (inside the META NCA): the string table is aligned to **0x20** (both titles: name 34 B → strtab=0x38, hdr=0x60 — the hacpack rule, unchanged in `packMetaNca`).
       `PFS0Writer` gains a `headerAlign` constructor arg (default 0x20 = legacy; all existing callers — split/merge/convert — keep the old behavior; the `inputStringTableSize` preserve-input branch is untouched). The six outer-NSP writers in `fs/update.js` now pass 0x10. Verified: output total = **701,770,512 B = yanu exactly**, PFS0 header 272 (0x110); Manual/Control byte-identical to yanu; Program/CNMT content unchanged (CNMT hash still `f2d47876…`); `test_convert.mjs` (NSZ→NSP) unaffected.
