 # NSZ to NSP Converter - Status Report
   ...
   6. **Perf: `--update` fully-streaming Program NCA pack — no RomFS/ExeFS-data buffer (seekable output)** — `fs/nca-pack.js`, `fs/update.js`, `fs/adapter.js`, `nsz-cli.js`, `scripts/test_update_e2e.mjs`. The last large RAM consumers of the merge path — the 578 MB merged RomFS and the 88 MB ExeFS data (both re-buffered to build the NCA and precompute its hash) — are gone for seekable outputs. New pieces:
     ...
     - `fs/update.js`: a streaming branch (used when `hasBktrRomfs && buildRead(output) != null`). ExeFS is still buffered (small; needed for `processNpdmAcid` ACID zeroing), the RomFS is streamed through `mergeRomFS(..., onChunk)`. The PFS0 header is written first with fixed-length placeholder names (Program = 32 zeros, CNMT = placeholder-contentId name — both fixed-length, so the header size is identical to the final one); after the re-read yields the real contentId, the PFS0 Program + CNMT names are patched in place (guarded by a header-size check). `programNcaSize()` precomputes the NCA size from the ExeFS/RomFS sizes (level-5 `hash_data_size` read from the update BKTR FsHeader @ 0x98).
     ...
     - Measured (Stardew v0 + v1310720, `/usr/bin/time -l`): peak RSS 3.8 GB → 1.65 GB → **0.60 GiB** (.nsp inputs, 14.4 s) / **1.36 GiB** (.nsz inputs, 21.6 s). Output **byte-identical** on both input formats: sha256 `f7a51189780a400b4cde0994284ff1d9fc3b009f0a10efc5783d2a1c920d0f95` (701,770,528 B). `test_convert.mjs` unaffected. The residual .nsp peak is the 88 MB ExeFS buffer (ACID zeroing) + Node's RSS high-water mark (V8 heap grows from the many transient 16 MB chunk allocations and does not immediately return it to the OS) — not a live data buffer.
