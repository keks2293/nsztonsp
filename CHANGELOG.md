...
2. **Perf: cache the `node:zlib` import** — `fs/ncz.js`. Both Node decompression call sites (the streaming `_decompressStream` and the NCZBLOCK `nextBlock`) previously did `await import('node:zlib')` on every call — per-chunk during streaming, per-block in NCZBLOCK. A module-scope `nodeZlibPromise = import('node:zlib')` is now resolved once and reused. NCZBLOCK benchmark (`bench_nczblock.mjs`, in-memory, best-of-5): ~1580 → ~2270 MB/s (≥+40%, within noise of the full emit). Behavior identical; `test_merge_ncz.mjs` passes.

3. **Perf: single-pass NCZBLOCK block schedule** — `fs/ncz.js`. `AsyncBlockDecompressorReader` previously built two parallel arrays (`compressedBlockSizeList`, `compressedBlockOffsetList`) across separate loops. It now builds one `blocks` array of `{relOffset, compressedSize, decompressedSize}` objects in a single pass, accumulating offsets in a running counter. One allocation and one loop instead of two of each; the last (short) block is handled inline. Behavior identical; `test_merge_ncz.mjs` passes; `bench_nczblock.mjs` ~2300 MB/s (no regression).

4. **Refactor: unify zstd platform dispatch in `crypto/zstd.js` + shared block decompression** — streaming and block decompression now have a single dispatch point each: `decompressStream(readChunk)` async-generator hosts both streaming seams (node:zlib `createZstdDecompress` push/write with concurrent output drain and the WASM `zstddec-stream-wrapper`), replacing the `if (isNode)` branches inline in `_decompressStream`; block mode reuses `decompressBlock(data)` (Node: in-process `node:zlib.zstdDecompressSync`, browser: `ZstdDecompressor` WASM, both Promise-based) from both `AsyncBlockDecompressorReader.nextBlock` and the exported module-level pure `parseBlockSchedule(...)` (per-block `relOffset`/`compressedSize`/`decompressedSize`, last block short; the raw/incompressible decision `compressedSize < decompressedSize` stays a synchronous inline check in `nextBlock` — no per-block microtask). `fs/ncz.js` no longer imports `node:zlib`/`zstddec-stream-wrapper.js` nor branches on the platform (its local `isNode`/`nodeZlibPromise` removed; `decompressStream` imported statically). Behavior identical; `test_merge_ncz.mjs` (streaming + NCZBLOCK) passes; `bench_nczblock.mjs` ~2400 MB/s.
